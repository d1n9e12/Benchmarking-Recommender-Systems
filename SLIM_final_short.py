...
def train_slim(config, checkpoint_dir = None):
    device = "cpu"
    if torch.cuda.is_available():
        device = "cuda:0"
        if torch.cuda.device_count() > 1:
            model = nn.DataParallel(model)
    slim = SLIMElasticNetRecommender(train_data)
    slim.fit(topK = config["topK"], l1_ratio = config["l1_ratio"], alpha = config["alpha"],
    ...
    for user in users:
        print(user)
        recon_batch = slim._compute_item_score(user)
...
config = { #What about the None
"topK" : tune.choice([None] + list(range(300))), 
"l1_ratio" : tune.choice([0.1, 0.2, 0.3, 0.4]),
"alpha" : tune.choice([0.5, 1., 1.5, 2.]),
"positive_only" : tune.choice([True, False]),
}

scheduler = ASHAScheduler(metric= "ndcg100", mode="max",) 
...
best_trial_slim = result_slim.get_best_trial("ndcg100", "max", "last")
print("Best trial config: {}".format(best_trial_slim.config))

print("Best trial final validation precision: {}".format(best_trial_slim.last_result["prec100"]))
print("Best trial final validation recall: {}".format(best_trial_slim.last_result["rec100"]))
print("Best trial final validation f1: {}".format(best_trial_slim.last_result["f100"]))
print("Best trial final validation hits: {}".format(best_trial_slim.last_result["hit100"]))

print("Best trial final validation ndcg: {}".format(best_trial_slim.last_result["ndcg100"]))
print("Best trial final validation mrr: {}".format(best_trial_slim.last_result["mrr100"]))
print("Best trial final validation arhr: {}".format(best_trial_slim.last_result["arhr100"]))
print("Best trial final validation map: {}".format(best_trial_slim.last_result["map100"]))
print("Best trial final validation mar: {}".format(best_trial_slim.last_result["mar100"]))


print("Best trial final validation novelty: {}".format(best_trial_slim.last_result["nov100"]))
print("Best trial final validation diversity: {}".format(best_trial_slim.last_result["div100"]))
print("Best trial final validation serendipity: {}".format(best_trial_slim.last_result["ser100"]))
print("Best trial final validation catalog coverage: {}".format(best_trial_slim.last_result["cc100"]))
print("Best trial final validation distributional coverage: {}".format(best_trial_slim.last_result["dc100"]))

best_slim = SLIMElasticNetRecommender(train_data)
best_slim.fit(topK = config["topK"], l1_ratio = config["l1_ratio"], alpha = config["alpha"],
                                                  positive_only = config["positive_only"])
