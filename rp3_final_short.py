...
def train_rp3beta(config, checkpoint_dir = None, number_epochs = 10, metrics_ks = [5, 10]):
    device = "cpu"
    ...
    rp3beta = RP3betaRecommender(train_data)
    rp3beta.fit(alpha = config["alpha"], beta =config["beta"],
            implicit = config["implicit"], #min_rating
            normalize_similarity = config["normalize_similarity"])
    ...
    for user in users:
        print(user)
        recon_batch = rp3beta._compute_item_score(user)
        ...
        mar100_l.append(map_sequence(recon_batch, labels, 100))
...
config = {"alpha" : tune.choice([0.5, 0.75, 1.]),
          "beta" : tune.choice([0.4, 0.5, 0.6, 0.7]),
          "topK" : tune.choice(list(range(300))), 
          "normalize_similarity" : tune.choice([True, False]),
          "implicit" : True
    } 
#"implicit" : tune.choice([True, False]),"min_rating" : tune.choice([1, 2, 3, 4, 5])
...
result_rp3beta = tune.run(
    partial(train_rp3beta),
...
best_trial_rp3beta = result_rp3beta.get_best_trial("ndcg100", "max", "last")
print("Best trial config: {}".format(best_trial_rp3beta.config))

print("Best trial final validation precision: {}".format(best_trial_rp3beta.last_result["prec100"]))
print("Best trial final validation recall: {}".format(best_trial_rp3beta.last_result["rec100"]))
print("Best trial final validation f1: {}".format(best_trial_rp3beta.last_result["f100"]))
print("Best trial final validation hits: {}".format(best_trial_rp3beta.last_result["hit100"]))

print("Best trial final validation ndcg: {}".format(best_trial_rp3beta.last_result["ndcg100"]))
print("Best trial final validation mrr: {}".format(best_trial_rp3beta.last_result["mrr100"]))
print("Best trial final validation arhr: {}".format(best_trial_rp3beta.last_result["arhr100"]))
print("Best trial final validation map: {}".format(best_trial_rp3beta.last_result["map100"]))
print("Best trial final validation mar: {}".format(best_trial_rp3beta.last_result["mar100"]))


print("Best trial final validation novelty: {}".format(best_trial_rp3beta.last_result["nov100"]))
print("Best trial final validation diversity: {}".format(best_trial_rp3beta.last_result["div100"]))
print("Best trial final validation serendipity: {}".format(best_trial_rp3beta.last_result["ser100"]))
print("Best trial final validation catalog coverage: {}".format(best_trial_rp3beta.last_result["cc100"]))
print("Best trial final validation distributional coverage: {}".format(best_trial_rp3beta.last_result["dc100"]))

best_rp3beta = RP3betaRecommender(train_data)
best_rp3beta.fit(alpha = best_trial_rp3beta.config["alpha"], beta = best_trial_rp3beta.config["beta"],
            #min_rating, implicit = False??
            normalize_similarity = best_trial_rp3beta.config["normalize_similarity"])
...
for user in users:
    recon_batch = best_rp3beta._compute_item_score(user)
    recon_batch[train_data[user].nonzero()] = -np.inf
    recon_batch = torch.FloatTensor(recon_batch)
