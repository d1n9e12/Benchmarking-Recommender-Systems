...
def train_userknn(config, checkpoint_dir = None):
    device = "cpu"
    ...
    userknn = UserKNNCFRecommender(train_data)
    userknn.fit(topK = config["topK"], shrink = config["shrink"],
                similarity = config["similarity"], normalize = config["normalize"],
                feature_weighting = config["feature_weighting"])
    ...
    for user in users:
        print(user)
        recon_batch = userknn._compute_item_score(user)
...
config = { #check for topK is None!!!
"topK" : tune.choice(list(range(300))), "shrink" : tune.choice([50, 100, 150, 200]),
"normalize" : tune.choice([True, False]),
"feature_weighting" : tune.choice(["BM25", "TF-IDF", "none"]),
"similarity" : tune.choice(["asymmetric", "cosine", "adjusted", "dice", "tversky"]),
}
...
result_userknn = tune.run(
    partial(train_userknn),
...
best_trial_userknn = result_userknn.get_best_trial("ndcg100", "max", "last")
print("Best trial final validation precision: {}".format(best_trial_userknn.last_result["prec100"]))
print("Best trial final validation recall: {}".format(best_trial_userknn.last_result["rec100"]))
print("Best trial final validation f1: {}".format(best_trial_userknn.last_result["f100"]))
print("Best trial final validation hits: {}".format(best_trial_userknn.last_result["hit100"]))

print("Best trial config: {}".format(best_trial_userknn.config))
print("Best trial final validation ndcg: {}".format(best_trial_userknn.last_result["ndcg100"]))
print("Best trial final validation mrr: {}".format(best_trial_userknn.last_result["mrr100"]))
print("Best trial final validation arhr: {}".format(best_trial_userknn.last_result["arhr100"]))
print("Best trial final validation map: {}".format(best_trial_userknn.last_result["map100"]))
print("Best trial final validation mar: {}".format(best_trial_userknn.last_result["mar100"]))


print("Best trial final validation novelty: {}".format(best_trial_userknn.last_result["nov100"]))
print("Best trial final validation diversity: {}".format(best_trial_userknn.last_result["div100"]))
print("Best trial final validation serendipity: {}".format(best_trial_userknn.last_result["ser100"]))
print("Best trial final validation catalog coverage: {}".format(best_trial_userknna.last_result["cc100"]))
print("Best trial final validation distributional coverage: {}".format(best_trial_userknn.last_result["dc100"]))

best_userknn = UserKNNCFRecommender(train_data)
best_userknn.fit(topK = best_trial_userknn.config["topK"], shrink = best_trial_userknn.config["shrink"],
                similarity = best_trial_userknn.config["similarity"], 
                normalize = best_trial_userknn.config["normalize"],
                feature_weighting = best_trial_userknn.config["feature_weighting"])
...
for user in users:
    recon_batch = best_userknn._compute_item_score(user)
    recon_batch[train_data[user].nonzero()] = -np.inf
    recon_batch = torch.FloatTensor(recon_batch)
